...
def test_session_without_afterthought_creates_no_internal_change(
    ...
    started = lifecycle.start(
        agent_id="lara", user_id="zhouwei", host="test", external_session_id="quiet"
    )
    ended = lifecycle.end(
    ...
    assert storage.recent_internal_events("lara") == []
    assert storage.pending_events("lara") == []
...
def test_unanswered_meaningful_question_can_be_afterthought(
    ...
    lifecycle = SessionLifecycle(storage, settings, backend)
    started = lifecycle.start(
        agent_id="clara", user_id="zhouwei", host="test", external_session_id="open"
    )
    ended = lifecycle.end(
        session_id=started["session"]["id"],
        agent_id="clara",
    ...
    assert ended["reflection"]["has_afterthought"] is True
    assert storage.pending_events("clara")[0]["content"]["open_loops"] == [
        "如何验证接续不是摘要复述"
    ]
...
def test_session_reflection_retries_once_on_bad_model_output(
    ...
    lifecycle = SessionLifecycle(storage, settings, FakeBackend(responder=responder))
    started = lifecycle.start(
        agent_id="lara", user_id="zhouwei", host="test", external_session_id="retry"
    )
    ended = lifecycle.end(
        session_id=started["session"]["id"],
        agent_id="lara",
...
def test_session_end_reports_concurrent_state_change(storage, tmp_path, monkeypatch):
    ...
    with storage.transaction() as conn:
        conn.execute(
            "UPDATE agent_state SET revision = revision + 1 WHERE agent_id = 'clara'"
        )
    ended = lifecycle.end(
        session_id=started["session"]["id"],
        agent_id="clara",
